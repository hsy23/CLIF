#!/usr/bin/env bash
# Live Qwen3 q_proj/v_proj LoRA smoke for the shared-base DeltaServe runtime.
# This is intentionally separate from the LM-head G5 sweep so its evidence is
# not mixed with the performance baseline.
#SBATCH --job-name=clif-ds-g6-live
...
OUTPUT="${OUTPUT:-${ROOT}/output/deltaserve-g6-live-${SLURM_JOB_ID:-local}.json}"
...
exec "$VENV/bin/python" "$ROOT/scripts/run_deltaserve_vllm_benchmark.py" \
  ...
  --baseline-requests "${BASELINE_REQUESTS:-4}" \
  --training-steps "${TRAINING_STEPS:-1}" \
  --training-batch-size "${TRAINING_BATCH_SIZE:-1}" \
  --training-sequence-length "${TRAINING_SEQUENCE_LENGTH:-96}" \
  --mixed-warmup-steps "${MIXED_WARMUP_STEPS:-0}"
